...
function main(){
  ...
  rl.question('Enter the title of the post: ', (title: string) => {
    if(title.trim() === ''){
      ...
      const postPath = `src/content/posts/${title.toLowerCase().replace(/ /g, '-')}`;
      // 检查目录是否存在
...
function createPostFile(data: {
  ...
  // 获取东八区时间
  const currentTime = dayjs().tz("Asia/Shanghai").format("YYYY-MM-DDTHH:mm:ssZ");
  const template = fs.readFileSync(`scripts/template/new-post/index.mdx`, "utf-8");
  const content = template.replace("{{title}}", title).replace("{{date}}", currentTime);
...
function createPreviewFile(data: {
  ...
  const template = fs.readFileSync(`scripts/template/new-post/preview.html`, "utf-8");
...
function createDataFile(data: {
  ...
  const dataTemplate = fs.readFileSync(`scripts/template/new-post/_src/data.json`, "utf-8");
  fs.writeFileSync(dataFileName, dataTemplate);

  const indexHtmlFileName = `${srcPath}/index.html`;
  const indexHtmlTemplate = fs.readFileSync(`scripts/template/new-post/_src/index.html`, "utf-8");
  fs.writeFileSync(indexHtmlFileName, indexHtmlTemplate);

  const indexCssFileName = `${srcPath}/index.css`;
  const indexCssTemplate = fs.readFileSync(`scripts/template/new-post/_src/index.css`, "utf-8");
  fs.writeFileSync(indexCssFileName, indexCssTemplate);

  const indexJsFileName = `${srcPath}/index.js`;
  const indexJsTemplate = fs.readFileSync(`scripts/template/new-post/_src/index.js`, "utf-8");
